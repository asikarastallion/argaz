...
PAGES = [
    ...
    # THE TWO GENERATED DOCUMENTS, IN THE PORTAL (v1.7)
    # -------------------------------------------------
    # These are the two files that say what has NOT been verified, and a portal
    # whose selling point is exactly that could not reach either of them: they
    # were readable from the README and from the repository, and not from the
    # Docs section. Recorded as F-27 in the v1.6 audit.
    #
    # They are machine output, so they carry a "DO NOT EDIT" banner — which the
    # portal strips, because an HTML comment is a generator instruction and not
    # content. English only, and deliberately: they are regenerated by CI from
    # the run directories, and a Turkish fork of a file a robot rewrites nightly
    # would be stale by the following morning. That is the same reasoning this
    # module states for every other untranslated page, and the reader is told.
    Page("test-coverage", "verification",
         {"en": "Test Coverage (generated)", "tr": "Test kapsamı (üretilmiş)"},
         {"en": "What is declared, what has run, and the mechanism matrix.",
          "tr": "Ne beyan edildi, ne koştu ve mekanizma matrisi."},
         source={"en": "docs/coverage.md"}, generated="coverage"),
    Page("verification-status", "verification",
         {"en": "Verification Status (generated)",
          "tr": "Doğrulama durumu (üretilmiş)"},
         {"en": "Which models were flown, by which tier, with what verdict.",
          "tr": "Hangi modeller, hangi katmanda, hangi hükümle uçuruldu."},
         source={"en": "docs/status.md"}, generated="status"),
    ...
    Page("simulation-lifecycle", "operations",
         {"en": "Simulation Lifecycle", "tr": "Simülasyon yaşam döngüsü"},
         {"en": "Environment ready, vehicle ready — and which layer a failure "
                "belongs to.",
          "tr": "Ortam hazır, araç hazır — ve bir arıza hangi katmana ait."},
         source={"en": "docs/simulation-lifecycle.md",
                 "tr": "docs/simulation-lifecycle.tr.md"}),
...
def _landing(lang: str) -> str:
    ...
    lines = list(header)
    for page in PAGES:
        # The landing page itself is the one page with no file behind it, so it
        # cannot appear in its own table of sources. Every OTHER page with a
        # `generated` name does have a file — it is written by a generator
        # rather than by a person — and it belongs in the list like any other.
        # This used to skip on `generated` being set at all, which would have
        # hidden the two documents that say what has not been verified.
