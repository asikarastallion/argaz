...
# 4 (v1.5): the document carries `coverage` — what is declared and has never
#           been run — so a reader of the status table is told what it leaves
#           out rather than being left to assume it covers everything.
SCHEMA = 4
...
def collect(runs_roots: list[Path], registry: Optional[dict] = None) -> dict:
    ...
    return {
        ...
        # What this project declares and has never run. Computed from the same
        # roots, so the two halves of the document can never disagree about
        # which runs they read.
        "coverage": coverage.collect(runs_roots, registry=registry),
    }
...
def render(data: dict, workflow: str = "", run_url: str = "") -> str:
    ...
    # -------------------------------------------------- what was NOT tested
    lines += _not_tested_section(data.get("coverage") or {})
...
def _not_tested_section(document: dict) -> list[str]:
    """What this project declares and has never run.

    WHY THIS IS IN THE STATUS TABLE AND NOT ONLY IN THE COVERAGE REPORT
    -------------------------------------------------------------------
    A verification status document that lists only what was tested is read as
    a list of everything there is. Every table above answers "what happened to
    the things we ran"; nothing in it can distinguish a project with four
    procedures that all pass from one with forty of which four were run.

    So the four dimensions are summarised here, named, in the same document as
    the verdicts — and the reader is pointed at the full lists rather than
    being given a percentage and left to assume the rest.
    """
    lines = ["## What was NOT tested", ""]
    if not document or not document.get("dimensions"):
        lines += ["No coverage document was available when this was generated, "
                  "so this section cannot say what was left out. That is itself "
                  "a gap: run `python3 -m argazui coverage`.", ""]
        return lines

    lines += [
        "Everything above is about things that were run. This is about the "
        "things that were not — which is the half a verification document is "
        "usually missing, and the half that decides how far its claims reach.",
        "",
        "| Dimension | Covered | Declared | Not covered |",
        "|---|---:|---:|---|",
    ]
    for dimension in document["dimensions"]:
        uncovered = dimension.get("uncovered") or []
        shown = ", ".join(f"`{item}`" for item in uncovered[:6])
        if len(uncovered) > 6:
            shown += f", and {len(uncovered) - 6} more"
        lines.append(f"| {dimension['label']} | {dimension['covered']} "
                     f"| {dimension['declared']} | {shown or '—'} |")
    lines += [
        "",
        "Full lists, with what each uncovered item is, are in "
        "[coverage.md](coverage.md). **Covered** there means some run "
        "exercised the item and produced a result — not that the result was a "
        "pass, and not that it was exercised more than once.",
        "",
    ]
    return lines
...
def generate(runs_roots: list[Path], out: Path, workflow: str = "",
             run_url: str = "", readme: Optional[Path] = None,
             coverage_out: Optional[Path] = None) -> str:
    roots = [Path(r) for r in runs_roots]
    data = collect(roots)
    text = render(data, workflow=workflow, run_url=run_url)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(text, encoding="utf-8")
    # The coverage document is written from the SAME collection, not a second
    # one: the status table summarises what was not tested and coverage.md
    # lists it, and two passes over the runs could produce two answers.
    target = coverage_out or (out.parent / "coverage.md")
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(coverage.render(data["coverage"]), encoding="utf-8")
