...
def test_the_documents_v13_added_exist_in_turkish():
    """Every subject this project wrote a document for has a Turkish source.
    ...
    this project having no Turkish twin would just be an omission.

    MACHINE OUTPUT IS THE ONE EXEMPTION, AND IT IS NAMED
    ----------------------------------------------------
    `docs/coverage.md` and `docs/status.md` are regenerated by `status.yml`
    from whatever runs are on disk, on its own schedule. A Turkish fork of a
    file a robot rewrites nightly would be stale by the following morning,
    which is the same argument this project already makes for not forking the
    schema references — a stale translation of a technical page is worse than
    an honest English one. The portal flags the fallback rather than letting it
    pass unremarked, which
    `test_a_generated_document_is_flagged_as_untranslated` asserts.

    Written as a set of page ids rather than a `generated` truthiness check, so
    adding a generated page is a deliberate edit here.
    """
    exempt = {"test-coverage", "verification-status"}
    for page in docs.PAGES:
        english = page.source.get("en", "")
        if not english.startswith("docs/"):
            continue
        if page.id in exempt:
            assert page.generated, (
                f"{page.id} is exempt from translation but is not generated")
            continue
...
def test_a_generated_document_is_flagged_as_untranslated():
    """The exemption above must not become a silent English fallback.

    A Turkish reader opening the coverage report gets the English file AND the
    notice saying so, exactly as they do for the changelog. An untranslated
    page this project chose is still a page it has to admit to.
    """
    for page_id in ("test-coverage", "verification-status"):
        document = docs.read(page_id, "tr")
        if not document.get("ok"):
            # The generated artefact is not present in this checkout, which is
            # a different problem from a translation gap.
            continue
        assert document["translated"] is False, page_id
        assert document["source_language"] == DEFAULT_LANG
        # And the Turkish title and summary ARE translated: the navigation a
        # reader browses is theirs even when the document is not.
        page = docs.BY_ID[page_id]
        assert page.title["tr"] != page.title["en"], page_id
        assert page.summary["tr"] != page.summary["en"], page_id
