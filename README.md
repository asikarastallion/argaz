...
*Verification status: **7** passed, **3** failed, **1** untested of 11 models ([docs/status.md](docs/status.md), generated 2026-08-06T12:55:10Z).*
<!-- /STATUS-SUMMARY -->
...
## Multi-vehicle simulation (v1.3)

One Gazebo world, N ArduPilot SITL instances, declared in a file rather than in
code. The single-vehicle path above is unchanged.

```bash
argazui fleet list                    # every spec
argazui fleet validate hexapod_trio   # check one WITHOUT starting anything
```

A fleet is `argazui/config/fleets/<name>.toml`. Validation is a gate, not a
warning: unique sysids in 1–255 (never 0), models that have actually passed
tier 2, one launch method per world, spawn points no closer than
`min_separation_m`, and a count under the ceiling. Two ship — `sitl_pair`
(Gazebo-free) and `hexapod_trio` (three vehicles in one world).

The interface gains a **Fleet** tab: vehicle grid, a target bar that says how
many vehicles the next command will reach *before* it is pressed, group
commands with a policy selector, the ACK matrix, separation and RTF panels,
and three terminals — a launch transcript, one attach-console for the focused
vehicle, and the existing shell. Not N terminals.

### A command result is not an ACK

ArduPlane was caught accepting a mode change and abandoning it 140 ms later,
with no NAK and no STATUSTEXT. So every per-vehicle result carries the
acknowledgement *and* a confirmation taken from later heartbeats:

| outcome | meaning |
|---|---|
| `ACCEPTED` | acked, and the state held |
| `REVERTED` | acked, and the state did **not** hold |
| `DENIED` | rejected, carrying the autopilot's own words |
| `TIMEOUT` | no acknowledgement in the window |
| `NO_LINK` | nothing to acknowledge it |

`REVERTED` exists because reporting it as `ACCEPTED` is a lie and reporting it
as `DENIED` is a different one.

### What it refuses to claim

- **Without Gazebo, separation is not measured at all.** Two free-running
  SITLs carry a clock offset of boot-stagger × speedup (0.90 s at speedup 1,
  4.51 s at speedup 5), so a distance computed across them is not a distance.
  The monitor emits nothing and the report says why.
- **Under Gazebo it is measured from `/world/<w>/pose/info`**, one message
  carrying every model's position under a single header stamp — not from each
  vehicle's MAVLink, whose clocks were measured to disagree by up to 0.32 s
  even under lockstep.
- **Every criterion has three outcomes**: passed, failed, and *not-measured*.
  A criterion that could not be evaluated never renders as a pass, and every
  report names what it did not claim.

### What it costs

Measured on a 16-core machine, vehicles hovering:

| vehicles | RTF mean | RTF min |
|---|---|---|
| 2 | 0.781 | 0.544 |
| 3 | 0.572 | 0.361 |
| 4 | 0.434 | 0.265 |

Throughput falls as roughly 1.77/N and is **not** limited by core count —
Gazebo steps physics serially and lockstep waits for each FDM in turn. Hence
`fleet.max_vehicles` defaults to a measured 4, not to a formula.

### Not implemented

Stated plainly, because a silent gap reads as a feature:

- HITL / real hardware bridge
- swarm control laws and formation flight (formations place vehicles at spawn;
  nothing holds them there afterwards)
- collision avoidance — separation is **detection only**
- remote access or authentication (localhost only)
- rover and boat models
- mixed launch-method fleets

Detail and the measurements behind each decision: [`docs/fleet-ports.md`](docs/fleet-ports.md),
[`docs/fleet-world-composition.md`](docs/fleet-world-composition.md),
[`docs/fleet-clock-drift.md`](docs/fleet-clock-drift.md),
[`docs/fleet-rtf-scaling.md`](docs/fleet-rtf-scaling.md),
[`docs/fleet-lockstep-stall.md`](docs/fleet-lockstep-stall.md),
[`docs/fleet-group-commands.md`](docs/fleet-group-commands.md),
[`docs/fleet-acceptance.md`](docs/fleet-acceptance.md),
