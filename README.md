...
*Verification status: **7** passed, **3** failed, **1** untested of 11 models ([docs/status.md](docs/status.md), generated 2026-08-03T10:16:10Z).*
<!-- /STATUS-SUMMARY -->
...
- **Class-based takeoff and landing procedures.** The flow for a copter, a
  plane, a quadplane and a tailsitter lives in declarative YAML
  (`argazui/procedures/`), not in code. The button and the regression test run
  the *same file*, so a passing test means a working button.
- **Acceptance criteria that measure.** A procedure passes when the aircraft
  actually did something — an altitude reached, a mode confirmed in a
  heartbeat, an attitude envelope held for the whole climb. An ACK is never a
  pass. This is what caught a tailsitter takeoff that had "passed" three times
  while the aircraft was tumbling at 1300°/s.
- **Installable somewhere else.** `argaz.toml`, `ARGAZ_*` environment
  variables or CLI flags decide where everything lives; no absolute path is
  baked in. `argazui doctor` checks the installation and every fix it prints
  runs verbatim.
- **Every flight leaves evidence.** `runs/<UTC>_<model>/` holds the dataflash
  log, a parameter dump with the run's own changes highlighted, the procedure
  that was executed verbatim, and a post-flight report with advisories.
- **A test suite that flies.** Real SITL, a real server, a real browser — no
  mocks anywhere. Tier 1 on every push; tier 2 flies the model set nightly.
- **Version-drift detection.** If the running server no longer matches what is
  on disk, the page says so and prints a command that restarts it.
...
**There is no hand-written list here any more.** There used to be, with eleven
green ticks against it and the sentence "Every model below was tested by
actually flying it". Nothing produced those ticks except somebody's belief, and
one of them was wrong for a year: the Plane takeoff button never worked.

The support table is now generated from test output:

### → [docs/status.md](docs/status.md)

It is written by CI after every nightly tier-2 run and after every tier-1 run,
and any hand edit to it is overwritten the next time. Each row names the
procedures that were flown, the firmware they flew on, when, and which tier
verified it.

**Support status is decided by machine output, not by human assertion.**
A result can only be one of four words:

| result | meaning |
|---|---|
| `passed` | flown in Gazebo; every acceptance criterion held |
| `failed` | flown in Gazebo; a step or a criterion did not hold |
| `flaky` | flown in Gazebo; passed only on the retry |
| `untested` | **not yet verified by a machine** |

`untested` does not mean broken and it does not mean working. It means nothing
has proven anything, which is a different — and more useful — statement than a
tick somebody typed.

**Expect the table to be smaller than the old one.** That is the point. Models
that were listed as fully working are now `untested` or `failed`, because the
old list was an assertion and this one is a measurement.

Rover, boat and walking-robot models in `SITL_Models` are intentionally out of
scope.

### Tier 1 makes no claim about any model

Two suites run, and only one of them may verify an airframe:

- **tier 1** — real ArduPilot SITL on its own generic frames, plus the server
  and the page in a real browser. Runs on every push in under ten minutes.
  It proves the *procedure logic and the application*, and it proves nothing
  about any model in the table above.
- **tier 2** — the real model set in Gazebo, headless, nightly. This is the
  only tier whose results appear as a model row.

A model tier 2 has not flown stays `untested` however green tier 1 is. If that
distinction ever blurs, the table becomes the thing it replaced.
...
v1.1 claims this and no more: **correct takeoff and landing procedures per
vehicle class, with measurable acceptance criteria; installable on another
machine; every flight leaves evidence; and support status is generated by
machine.**

Deliberately not in this version, and not started:

- multi-vehicle / swarm simulation
- HITL (a bridge to real hardware)
- scenario YAML — the `mission:` and `failures:` keys are reserved in the
  procedure schema and rejected until then
- failure injection (wind, GPS loss, motor failure through `SIM_*`)
- regression comparison between runs
- authentication, remote access, graphical mission planning and telemetry
