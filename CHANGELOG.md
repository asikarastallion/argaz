...
One Gazebo world, N ArduPilot SITL instances, declared in a file, launched
through readiness gates, commanded with per-vehicle results, measured where
measurement is possible and explicitly silent where it is not.

A fleet is a TOML file in `argazui/config/fleets/`. `argazui fleet validate
<name>` checks one without starting anything: unique sysids in 1–255, models
that have actually passed tier 2, one launch method per world, spawn points no
closer than `min_separation_m`, and a vehicle count under the ceiling. Two
ship: `sitl_pair` (Gazebo-free, what CI runs) and `hexapod_trio` (three
vehicles in one world).

The interface gains a **Fleet** tab. The single-vehicle page is byte-for-byte
unchanged, and `tests/test_characterisation.py` exists to keep it that way.

### What was measured, and what it changed

Nearly every design decision in v1.3 was made by a measurement that
contradicted the plan.

**Ports.** `-I i` shifts SERIAL0 to `5760 + 10i` and the FDM port to
`9002 + 10i`, automatically. There is no `9003 + 10i`: the JSON backend
ignores its inbound port and receives on the same unbound socket it sends
from. **`-I` does not set the sysid** — three instances launched without
`--sysid` all report sysid 1, so every "addressed" command would reach all of
them. And SITL blocks at boot until something connects to SERIAL0, which under
lockstep means one unattached vehicle stalls the whole world; the fleet
launches with `--serial0 tcp:0` so each vehicle boots independently.
(`docs/fleet-ports.md`)

**World composition.** Putting a `<plugin>` override inside `<include>` looks
elegant and is a trap: it ADDS a second `ArduPilotPlugin` instead of replacing
the model's own, so every vehicle keeps a shadow plugin on 9002 — and two
plugins on one UDP port both bind successfully, with no error. The first test
of this appeared to pass only because one override value happened to equal the
built-in default. Per-vehicle model materialisation is what actually works.
(`docs/fleet-world-composition.md`)

**Position reference.** `ArduPilotPlugin` reports the model's WORLD pose, so
the Gazebo `<pose>` already carries the spawn offset. Giving SITL an offset
home as well would double it — and Gazebo would still draw the vehicle in the
right place, so the error would show up only as a separation trace that
disagrees with the picture. Under Gazebo the whole fleet shares one home;
without Gazebo there is no pose, so the offset goes into `--home` per vehicle
instead. Same principle, two channels.

**Real-time factor, and why the ceiling is 4.** Measured with vehicles
hovering, on a **16-core** machine:

| vehicles | RTF mean | RTF min | sim s / wall s |
|---|---|---|---|
| 2 | 0.781 | 0.544 | 0.886 |
| 3 | 0.572 | 0.361 | 0.607 |
| 4 | 0.434 | 0.265 | 0.489 |

Throughput falls as roughly **1.77/N**. It is not limited by cores — Gazebo
steps physics serially and lockstep waits for every FDM in turn, so adding
cores does not buy vehicles. `fleet.max_vehicles` was `max(2, cores // 2)`,
which allows eight here and extrapolates to about 0.22x. It is now a measured
**4**. (`docs/fleet-rtf-scaling.md`)

**Clock drift, and why MAVLink-derived separation is refused.** Without
Gazebo, two SITLs carry a constant offset of boot-stagger × speedup: 0.90 s at
speedup 1, 4.51 s at speedup 5. Their rates agree; the offset does not shrink,
it scales. Under Gazebo lockstep it improves but does not vanish — up to
**0.32 s** between three vehicles, structured and stable, with v1 sitting
0.28 s behind the world clock and v3 0.30 s ahead for a whole run. Sampling
the streams more carefully produced a *larger* number, which settles it.

So separation is **not** computed from each vehicle's MAVLink position. It is
read from `/world/<w>/pose/info`, one message carrying every model's position
under a single header stamp — simultaneous by construction rather than by an
assumption about clocks. Without Gazebo the monitor emits nothing at all, and
the run says so. (`docs/fleet-clock-drift.md`)

**The lockstep stall.** Induced deliberately with `SIGSTOP`. Two assumptions
were wrong: it does not stop cleanly (sim time crawls at ~0.04x while the
plugin drains buffered packets), and when it truly stalls `gz topic /stats`
**stops answering** — the server is blocked inside the plugin's wait loop. A
physics server that is running but silent is the strongest stall signal there
is; the first detector read it as "no measurement available" and reported
healthy. It now names the frozen vehicle by process state and says explicitly
that the others are silent as a consequence, not as a cause.
(`docs/fleet-lockstep-stall.md`)

### A command result is not an ACK

Phase 3 caught ArduPlane accepting a mode change and abandoning it 140 ms
later — no NAK, no STATUSTEXT, the mode simply back where it started. So every
per-vehicle result carries two findings: the acknowledgement, and a
confirmation taken from later heartbeats and held for a stated interval. Five
outcomes, not two:

    ACCEPTED   acked, and the state held
    REVERTED   acked, and the state did NOT hold
    DENIED     rejected, with the autopilot's own words
    TIMEOUT    no acknowledgement in the window
    NO_LINK    nothing to acknowledge it

`REVERTED` existing as its own outcome is the point. Reported as ACCEPTED it
is a lie; reported as DENIED it is a different one, because the autopilot did
accept it. Both real failures were induced against real autopilots — a
permanent pre-arm refusal, and a mode undone by the flight-mode switch.
(`docs/fleet-group-commands.md`)

### Acceptance is judged on time, and has three outcomes

Following v1.1's attitude rule: a peak is one sample. Threshold criteria ask
how many **seconds** were spent outside the band against a declared tolerance,
never the worst sample — the project's own data has a run with RTF min 0.265
and median 0.42, which a minimum-keyed criterion cannot tell from one that sat
at 0.265 throughout.

And a criterion has three outcomes: passed, failed, **not-measured**. A
criterion that could not be evaluated never renders as a pass, the run verdict
has a matching `INCOMPLETE`, and every report carries a section naming what it
did not claim. (`docs/fleet-acceptance.md`)

### No threshold is a constant

Three fixed numbers were wrong in one phase, twice by accusing a working
system: a 0.30 m "stationary" band (a hovering vehicle drifts 0.47 m), an 8 s
settle (at 0.6x that is under 5 s of flight), and a 1.5 s wall-clock hold
(7.5 vehicle-seconds at speedup 5, against a 10 s auto-disarm). Every
threshold is now measured from a floor, expressed in vehicle time, or replaced
by waiting on a condition. (`docs/thresholds.md`)

### Fixed: a mode command could be silently discarded

The single-vehicle flight test failed 3 times in 7 runs, and the cause was not
the test. ArduPlane reads its flight-mode switch shortly after RC input
becomes valid and overwrites any mode commanded in that ~100 ms window,
without a NAK. The interface enabled its buttons as soon as the link was up,
so a person clicking quickly hit the same race. Command buttons now wait for
the mode to have been still for 2 seconds of **vehicle** time.

### Not implemented

Listed explicitly, because a silent gap reads as a feature:

- **HITL / real hardware bridge** — no autopilot board is in the loop.
- **Swarm control laws / formation flight** — formations place vehicles at
  spawn; nothing keeps them in formation afterwards.
- **Collision avoidance** — separation is **detection only**. Nothing steers
  to avoid anything.
- **Remote access / authentication** — localhost only, as before.
- **Rover and boat models** — copters and planes only.
- **Mixed launch-method fleets** — one `launch_method` per world, enforced by
  the validator with a message naming which vehicle wants which.

### Also

- `fleet_sitl` runs on every push; `fleet_gazebo` runs nightly. Neither can
  produce or alter a model row in `docs/status.md`, which is asserted by test
  and verified by mutation.
- Each tier writes its own suite record. One shared `suite.json` silently
  erased a tier-2 result mid-development and reverted fleet validation to
  `untested`.
- Per-vehicle PlotJuggler namespacing (`v2/ATTITUDE/roll`). Empty by default,
  so the single-vehicle stream is unchanged.
